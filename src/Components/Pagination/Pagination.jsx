...
const Pagination = () => {
  ...
  const baseRoute = idGenre ? "/categories" : "/movies"

  return (
    <div className="flex items-center w-full max-w-[80%] justify-center flex-wrap gap-[2rem] mb-[5rem]">
      {[...Array(6)].map((_, index) => {
        const pageNumber = index + 1;
        const route = idGenre ? `${baseRoute}/${pageNumber}/${idGenre}` : `${baseRoute}/${pageNumber}`
        ...
        return (
          ...
          no-underline transion-all duration-400 hover:bg-[#fff] hover:text-[#f4b315]`} to={route}>{pageNumber}</Link>
        );
      })}
    </div>
  );
